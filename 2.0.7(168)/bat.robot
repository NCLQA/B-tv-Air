...
50463 VOD 월정액 콘텐츠 재생 시 LGS 로그의 ppm_ids 정보 확인
...
50464 VOD 재생 > LGS 로그의 pid 정보 확인
    ...
    ${log}    Run Process    adb logcat -d 15 >> D://RFA/log.txt    shell=True
    ${String}    Get File    D://RFA/log.txt    encoding=ISO-8859-1
    Should Contain    ${String}    "pid":"413252607"
    Should Contain    ${String}    "ppm_ids":""

31210 패키지 게이트웨이 진입 확인
...
31231 가입된 월정액 VOD 재생 확인
    #0. CJ ENM 가입 / 검증 콘텐츠 <세작, 매혹된 자들>
    #1. 앱 실행 > "검색" > 가입된 월정액에 포함된 콘텐츠 검색 > 재생    /    1. 본편 재생 로그("event_type":"3") 출력 시 pass

    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skb.smartrc.air:id/icSearch
    Sleep    1
    Wait Until Element Is Visible    com.skb.smartrc.air:id/top_search_bar
    Wait Until Keyword Succeeds    1 min    3 sec    Input Text    com.skb.smartrc.air:id/top_search_bar    세작 매혹된 자들
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.TextView[@text = '세작 매혹된 자들']
    Sleep    3
    Run Process    adb logcat -c    shell=True
    Run Keyword And Ignore Error    Remove File    D://RFA/log.txt

    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.view.ViewGroup[@content-desc="세작, 매혹된 자들"]/android.view.ViewGroup/android.widget.ImageView[1]
    Sleep    10
    ${log}    Run Process    adb logcat -d 15 >> D://RFA/log.txt    shell=True
    ${String}    Get File    D://RFA/log.txt    encoding=ISO-8859-1
    Should Contain    ${String}    "event_type":"3"
    
68644 성인 인증 번호 동작 확인
    ...
    비밀번호 입력 (1111)
    Sleep    10
    ${log}    Run Process    adb logcat -d 15 >> D://RFA/log.txt >> D://RFA/log.txt    shell=True
    ${String}    Get File    D://RFA/log.txt    encoding=ISO-8859-1
