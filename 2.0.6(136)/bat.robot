...
예고편, 미리보기 동작 확인
    ...
    #2. 컴포넌트 호출    /    2. '3분 미리보기' 문구 노출 시 pass
    ...
    Sleep    5
    ${log}    Run Process    adb shell logcat -t 5000 >> D://RFA/log.txt    shell=True
    ${String}    Get File    D://RFA/log.txt    encoding=ISO-8859-1
    Should Contain    ${String}    processPreviewStart

    FOR  ${i}  IN RANGE    0    10    #3분 미리보기 문구 노출 시까지 컴포넌트 호출 반복
        Swipe By Percent    50    20    50    20
        ${preview}    Run Keyword And Return Status    Page Should Contain Element    com.skb.smartrc.air:id/tv_preview
        Exit For Loop If    ${preview}
    END
