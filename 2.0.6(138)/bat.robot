...
VOD 월정액 콘텐츠 재생 시 LGS 로그의 ppm_ids 정보 확인
    ...
    Sleep    10
    ${log}    Run Process    adb logcat -s OkHttp -t 100000 >> D://RFA/log.txt    shell=True
...
예고편, 미리보기 동작 확인
    ...
    Sleep    10
    ${log}    Run Process    adb logcat -s [btv] -t 100000 >> D://RFA/log.txt    shell=True
    ${String}    Get File    D://RFA/log.txt    encoding=ISO-8859-1
...
성인 인증 번호 동작 확인
    ...
    비밀번호 입력 (1111)
    Sleep    10
    ${log}    Run Process    adb logcat -s OkHttp -t 100000 >> D://RFA/log.txt >> D://RFA/log.txt    shell=True
    ${String}    Get File    D://RFA/log.txt    encoding=ISO-8859-1
